...
void KMeans::fit(const std::vector<Point>& points, const std::vector<Point>& start_points) {
...
void KMeans::classify(const std::vector<Point>& points, std::vector<uint8_t>& cluster) {
...
size_t KMeans::compare(const std::vector<uint8_t>& first, const std::vector<uint8_t>& second,
...
void KMeans::load_clusters(std::ifstream& ifs) {
  std::string line;
  while (std::getline(ifs, line)) {
    Point centroid;
    std::stringstream ss(line);
    ss >> centroid.x >> centroid.y;
    m_cluster_centers.push_back(centroid);
  }
}

void KMeans::write_clusters(std::ofstream& ofs) {
  for (const Point& p : m_cluster_centers) {
    ofs << p.x << " " << p.y << "\n";
  }
}
